...
int main( int argc, char** argv )
{
	...
	while(getNextImage(src_bgr, src_dep))
	{
		...
		/// ellipse detection
		RotatedRect eDetect;
		Point3f currPos, currPosImg, currPosWorld, nextPos, nextPosAll;
		...
		if(detectEllipse(cFilter, eDetect, 150, 7, 7) == 1) {
			...
			currPosWorld = conv.cameraTOworld(currPos);
